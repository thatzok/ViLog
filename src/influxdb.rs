...
pub async fn send_to_influx(client: reqwest::Client, influx: Arc<InfluxResolved>, body: String) -> Result<(), reqwest::Error> {
    ...
    let mut req = client.post(full_url)
        .query(&[("org", influx.org.as_str()), ("bucket", influx.bucket.as_str()), ("precision", "ms")])
